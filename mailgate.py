...
class email_parser(object):
    ...
    def party_get(self, email):
        mail = self.email_get(email)
        adr_ids = self.rpc('res.party.address', 'search', [('email', '=', mail)])
        if not adr_ids:
            return {}
        adr = self.rpc('res.party.address', 'read', adr_ids, ['party_id'])
        return {
            'party_address_id': adr[0]['id'],
            'party_id': adr[0]['party_id'][0]
        }
    ...
    def msg_new(self, msg):
        ...
        try:
            data.update(self.party_get(self._decode_header(msg['From'])))
        except Exception, e:
            import netsvc
...
#   #
    ...
    def msg_user(self, msg, id):
        ...
        if 'party' in actions:
            data['email_from'] = actions['party'][:128]

        if 'user' in actions:
            uids = self.rpc('res.user', 'name_search', actions['user'])
            if uids:
